...
def get_packet_value(packet):
    ...
    #I don't have python 3.10 on my distro yet, so no match. Feelsbadman. 
    if packet.packetType == 4:
        return packet.value
    
    elif packet.packetType == 0:
        return sum(map(get_packet_value, packet.subs))

    elif packet.packetType == 1:
        product = 1
        for sub in packet.subs:
            product *= get_packet_value(sub)
        return product

    elif packet.packetType == 2:
        return min(map(get_packet_value, packet.subs))

    elif packet.packetType == 3:
        return max(map(get_packet_value, packet.subs))

    elif packet.packetType == 5:
        subValues = list(map(get_packet_value, packet.subs))
        ...
        raise(Exception(f"Invalid Packet Type: {packet.packetType}"))
...
def main():
    ...
    if len(packets) == 1:
        print(get_packet_value(packets[0]))
    else:
